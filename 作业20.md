# 题目
## 给定一个轨迹１，数据格式：timestamp tx ty tz qx qy qz qw， 自定义一个任意的旋转矩阵和平移向量（可以尝试不同的值，甚至加一些噪声看看结果有什么变化），对轨迹１进行变换，得到一个新的轨迹２， 使用ICP算法（提示：取平移作为三维空间点）估计轨迹１，２之间的位姿，然后将该位姿作用在轨迹２。
